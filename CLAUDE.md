...
A static web app (`site/`, plain HTML + CSS + JS, served by the zero-dependency `scripts/serve.mjs`) renders each project's `output/`. **Nothing to install, nothing to build**: it starts in a second on any machine with Node 20+, which is the point — the old Next.js `npm install` blocked participants for up to 30 minutes on corporate networks. **In-app browser first:** when the `mcp__Claude_Browser__*` tools are available (Claude Desktop), launch and open the app with `preview_start` `{name: "web"}` (config in `.claude/launch.json`) — the page lives right next to the chat. Otherwise run `npm run web:dev` → http://localhost:3000, then **open the browser for the participant** (`open`/`Start-Process`/`xdg-open`) — never just print a localhost link, many won't click it. During the design interview the agent flow is shown **in the chat** (compact ASCII, redrawn after each answer) and mirrored to the **/flow** page (`agent-flow.mmd`, Mermaid, repolled every 2s) for a projector or second screen. Upgrading the app with `showcase` at the end of the lab is a demo moment.

## Tooling

- **Environment check:** `npm test` (see `test-repo` skill) — run it if anything seems broken. ~5s, no install needed.
- **Optional install:** `npm install` at the root, in the background, only for decks and PDF reading. Never `npm --prefix web install` during a lab (that's the Next.js fallback in `web/`, instructor-only: `npm run web:next`).
